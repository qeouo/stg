...
var Ono3d = (function(){
	...
	var calcLighting = ret.calcLighting = function(normal,lightSources){
		...
		for(i=lightSources.length;i--;){
			lightSource = lightSources[i]
			if(lightSource.type ==LT_DIRECTION){
				dot=-Vec3.dot(lightSource.viewAngle,normal)
				if(dot>0) light +=dot*lightSource.power;
...
//					target.fillRect(vertices[0].screenx-(b>>1)-0.5,vertices[0].screeny-(b>>1)-0.5,b,b)
		...
		,clear: function(){
			...
			this.renderVertices_index=0
			if(drawMethod==9){
				if(this.targetImageData){
					Rastono.clear(this.targetImageData);
				}
			}else{
				if(this.renderTarget){
					this.renderTarget.clearRect(0,0,this.rendercanvas.width,this.rendercanvas.height);
				}
			}
